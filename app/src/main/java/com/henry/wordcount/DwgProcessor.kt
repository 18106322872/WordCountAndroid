...
object DwgProcessor {
    ...
    private suspend fun processInner(context: Context, file: File, dName: String): DwgProcessResult {
        ...
        try {
            ...
            // v1.9.32: 进程崩溃(-96)是可恢复失败——重启隔离进程后对该文件重转一次，
            // 避免单个文件 native 崩溃在切后台时使后续文件连锁 -97(startForegroundService 受限) 全挂。
            // -97/-98/-99 重试无意义：-97 为后台前台启动受限(重试必再失败)，-98/-99 为超时/发送失败。
            var pyDxfRes = DwgIsolatedRunner.convertToDxf(context, file.absolutePath, pyDxfPath)
            if (pyDxfRes.path == null && pyDxfRes.errorCode == -96) {
                Log.w("WordCount", "DWG 转换进程崩溃 rc=-96，重启隔离进程重试 $dName")
                try { DwgIsolatedRunner.stopIsolated(context) } catch (_: Throwable) {}
