        ...
        if ((nonChineseDxf || encodingLoss) && !recoverySucceeded && !oleApplied) {
            ...
            try {
                ...
                if (oleRes.text.isNotBlank()) {
                    ...
                    // v1.8.6: 纯英文/编号图纸(nonChineseDxf)的 OLE 预览通常也会识别出
                    // 大量块炸开编号，不能当真字计入。仅当 OLE 能补回真实中文(fe>0)时才采用；
                    // 否则保持丢弃状态，走 needsPdf 显示"-"。
                    if (oleStats.fourth > 0 && (!nonChineseDxf || oleStats.second > 0)) {
                        ...
                        Log.d("WordCount", "DWG OLE OCR 拒用(纯编号噪声) $dName: fe=${oleStats.second} nc=${oleStats.third}")
