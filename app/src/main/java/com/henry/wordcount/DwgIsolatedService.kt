...
class DwgIsolatedService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        // v2.1.9: 初始化 Diag，使本隔离进程的日志落盘到 wc_diag_<procName>.log，诊断导出即可抓到 :dwgisolated 段落
        Diag.init(applicationContext)
        messenger = Messenger(IncomingHandler())
        Diag.d("isolated service created (pid=${android.os.Process.myPid()})")
    }
    ...
    private fun startForegroundCompat() {
        ...
        if (wakeLock == null) {
            try {
                ...
                Diag.w("acquire wakelock failed: ${e.message}")
            }
        }
        ...
        try {
            ...
            Diag.w("startForeground 失败 → stopSelf: ${e.message}")
            ...
            when (msg.what) {
                MSG_CONVERT_DXF -> {
                    ...
                    try {
                        ...
                        Diag.e("convert(dwg2dxf) threw: ${e.message}", e)
                ...
                MSG_CONVERT -> {
                    ...
                    try {
                        ...
                        Diag.e("convertToPdf threw: ${e.message}", e)
                        bundle.putInt(KEY_RC, -99)
                        bundle.putString(KEY_DIAG, "exception: ${e.message}")
                        bundle.putString(KEY_PATH, null)
                    }
