...
object PdfOcrEngine {
    ...
    // v1.9.152: 内嵌图片 OCR 尺寸上限。P403051 单张内嵌图原生 29195×897，

    //   直接解码会 OOM；降采样到 5000px 后文字仍清晰（桌面 RapidOCR 实测 6465 字 = 桌面真值 6368 字口径），

    //   且像素数只有整页渲染 5000×3536 的 ~40%，块数 4 vs 12，时间 ~1/3（~100s，远在 240s 超时内）。

    private const val EMBEDDED_IMAGE_MAX_DIM = 5000
    // v1.9.168: 原生切片 OCR 仅针对「横向超宽拼接扫描件」（宽>高 且 宽/高 >= 此阈值），
    //   排除普通竖向扫描页（如 AH+(1).pdf 每页 1 张 ~2748x3920 竖向图，aspect≈0.7）。
    //   P403051 类由 29195x897 横向条组成（aspect≈32），必须保留原生切片 OCR。
    private const val EMBEDDED_SLICE_MIN_ASPECT = 4.0f
    // v1.9.168: 2x 基准渲染长边上限。AH+(1).pdf 类「页 MediaBox=图片像素尺寸」的扫描件，
    //   2x 渲染会生成 ~5800x8700 巨型位图，PaddleOCR 串行极慢（单页 240s 超时）。
    //   钳到 2600px 后 PaddleOCR 秒级完成；扫描件最终以 ML Kit 6x 为准（见 renderAndRecognizeStrong）。
    private const val BASE_RENDER_MAX_LONG_PX = 2600
                ...
                val futures = (0 until limit).map { i ->
                    ...
                    pdfOcrPool.submit(java.util.concurrent.Callable {
                        ...
                        try {
                            ...
                            val bmp = renderPageSysBitmap(file, i, forPrintMode, null, BASE_RENDER_MAX_LONG_PX)
                                ...
                                pageBase[i] = baseText   // v1.9.133: 发布 2x 基准，供超时兜底
                                    ...
                                    if (baseText.length < ADAPT_MIN_CHARS) {
                                        ...
                                        if (embText.length >= ADAPT_MIN_CHARS) {
                                            ...
                                            Diag.d("PdfOcr p${i+1}: 内嵌图片OCR 仅 ${embText.length}字(<${ADAPT_MIN_CHARS})，不采用，回退 6×")
                                    ...
                                    // v1.9.171: 扫描件对齐桌面 2x，不渲染 6x upBmp（省去原生 6x 切片耗时）
                                    val upBmp = if (isScanPdf) null else renderPageSysBitmap(file, i, forPrintMode, ADAPT_SCALE, UPSCALE_MAX_LONG_PX)
                                    if (isScanPdf) Diag.d("PdfOcr p${i+1}: [扫描件] 对齐桌面 2x 基准，跳过 6× 升采样，当前 ${bestText.length}字")
                                    ...
                                    if (upBmp != null) {
                                        ...
                                        try {
                                            ...
                                            if (!isBlankBitmap(upBmp)) {
                                                ...
                                                if (bestText.length >= ADAPT_MIN_CHARS) {
    ...
    private fun extractEmbeddedImages(file: File, maxDim: Int = 0): List<Bitmap> {
        ...
        try {
            ...
            strategyA_XObjectImage(data, results, seenOffsets, maxDim)

            if (results.isEmpty()) strategyB_RelaxedImage(data, results, seenOffsets, maxDim)

            if (results.isEmpty()) strategyC_RawStreamDecode(data, results, seenOffsets, maxDim)
    ...
    // v1.9.152: 模块级强引擎识别（与嵌套 recognizePageStrong 同口径，但可独立调用）。

    //   对内嵌拼接大图做 PaddleOCR：反色 + 增强 + 分块识别 + 中文噪声过滤。返回过滤后文本。

    //   不回收入参 bmp（由调用方负责），与 recognizePageStrong 行为一致。

    private fun recognizeBitmapStrongText(bmp: Bitmap): String {
...
private fun ocrNativeSlice(raw: ByteArray): String {
    ...
    for (sm in subPat.findAll(str)) {
        ...
        try {
            ...
            // v1.9.168: 仅收集横向超宽切片（宽>高 且 宽/高 >= EMBEDDED_SLICE_MIN_ASPECT），
            //   排除普通竖向扫描页（AH+(1).pdf 每页 2748x3920 竖向图 aspect≈0.7，不应走原生切片 OCR）。
            val oh = opts.outHeight
            if (oh <= 0 || opts.outWidth.toFloat() / oh < EMBEDDED_SLICE_MIN_ASPECT) continue
...
private fun strategyA_XObjectImage(data: ByteArray, out: MutableList<Bitmap>, seen: MutableSet<Int>, maxDim: Int = 0) {
        ...
        for (match in imgPattern.findAll(str)) {
            ...
            try {
                ...
                val bmp = BitmapFactoryDecode(imgBytes, maxDim) ?: continue
    ...
    private fun strategyB_RelaxedImage(data: ByteArray, out: MutableList<Bitmap>, seen: MutableSet<Int>, maxDim: Int = 0) {
    ...
    private fun strategyC_RawStreamDecode(data: ByteArray, out: MutableList<Bitmap>, seen: MutableSet<Int>, maxDim: Int = 0) {
        ...
        while (pos <= data.size - kw.size - 2 && attempts < maxAttempts && out.size < MAX_PAGES) {
            ...
            if (dataLen > 1024 && dataLen < 10 * 1024 * 1024 && !seen.contains(dataStart)) {
                ...
                try {
                    ...
                    val bmp = BitmapFactoryDecode(chunk, maxDim)
